...
# -include cassert + cstdint: GCC-14 no longer transitively provides <cassert>/<cstdint>;
# duneana uses assert() / uint32_t (e.g. DAQSimAna/PackedDump/read_packed.cxx) without
# including them -> "'assert'/'uint32_t' not declared".
export CXXFLAGS="${CXXFLAGS:-} -fpermissive -include cassert -include cstdint"

# rootcling does NOT see the compiler's -include flags, so the CalibAna ROOT dictionary
# fails on uint16_t (CalibAnaTreeObj.h uses uint16_t but includes only <climits>/<limits>,
# not <cstdint>). Patch the header directly so both the compiler and rootcling see it.
sed -i '/^#define DUNEANA_CalibAnaTreeObj/a #include <cstdint>' \
  "$SRC_DIR/duneana/CalibAna/CalibAnaTreeObj.h"
