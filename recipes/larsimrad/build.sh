#!/bin/bash
# larsimrad: LArSoft radiological-background sim (BxDecay0 wrapper). cetmodules
# build above larsim.
set -euo pipefail
...
# find_package(BxDecay0 ... HINTS $ENV{BXDECAY0_FQ_DIR}) -> conda-forge bxdecay0.
export BXDECAY0_FQ_DIR="$PREFIX"

# rstartree (via larreco closure) + the generator env contracts re-exported
# transitively through larsimConfig/nugenConfig (find_dependency GENIE/MARLEY/...).
