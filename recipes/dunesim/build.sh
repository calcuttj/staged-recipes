...
# -include cassert: several dunesim DetSim tools/services use assert() without
# #include <cassert> (GCC-14 no longer pulls it in transitively) -> "'assert' was not
# declared". Force-include it into every TU rather than patch each file.
