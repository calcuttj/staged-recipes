...
# It is a self-contained analysis module — nothing in triggeralgs/TriggerSim/
# channelmaps or the other TriggerAna modules depends on it. art_make globs *_module.cc,
# so drop just this module's source (the other 3 TriggerAna modules + TriggerSim +
# triggeralgs still build). The trigger *simulation* the stack needs is unaffected.
rm -f "$SRC_DIR/dunetrigger/TriggerAna/TriggerAnaTree_module.cc" \
      "$SRC_DIR/dunetrigger/TriggerAna/TriggerAnaTree_module.hh"
