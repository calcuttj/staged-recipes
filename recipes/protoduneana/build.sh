...
# FitUtils/CMakeLists uses $ENV{VALGRIND_INC}; point it at the conda valgrind headers.
export VALGRIND_INC="$PREFIX/include"

# protoduneana FitUtils/MCToyGenerationAndFit.cxx has a vestigial `#include
# "RooChi2Var.h"` — RooChi2Var was removed from RooFit's public API in ROOT 6.36 (it
# exists in 6.28). The class is NOT actually used in the file (only the include), so the
# header is missing at compile with no other effect. Drop the dead include.
sed -i '/#include "RooChi2Var.h"/d' "$SRC_DIR/protoduneana/Utilities/FitUtils/MCToyGenerationAndFit.cxx"

# protoduneana/Utilities/FitUtils (ProtoDUNE cross-section fitting: runProtoDUNEFit,
# runPDSPThinSliceFit, ...) uses RooFit APIs REMOVED in ROOT 6.36 — RooArgSet::
# createIterator(), RooCatType, RooArgList::iterator — at 35 sites across
# ProtoDUNEFitUtils.cxx / MCToyGenerationAndFit.cxx / ProtoDUNEFit.cxx. The old
# TIterator-based RooFit iteration was dropped in 6.36 (needs a range-for port). The
# `protodune_fitutils` library is linked by NOTHING outside FitUtils, and FitUtils is
# its own subdir, so drop it to get protoduneana GREEN under ROOT 6.36. This EXCLUDES
# the ProtoDUNE fit executables — TODO: port the RooFit 6.36 iteration API and re-add.
sed -i '/add_subdirectory( *FitUtils *)/d' "$SRC_DIR/protoduneana/Utilities/CMakeLists.txt"
